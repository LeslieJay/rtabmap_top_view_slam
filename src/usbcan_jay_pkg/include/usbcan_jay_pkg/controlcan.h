 ...
 * @brief 打开CAN设备
 * 
 * 此函数用于打开指定类型和索引的CAN设备。一个设备只能被打开一次，重复打开会失败。
 * 
 * @param DeviceType 设备类型（如USBCAN2为4等）
 * @param DeviceInd  设备索引号（如0、1等）
 * @param Reserved   保留参数，通常填0
 * @return DWORD     返回值为1表示成功，0表示失败
 */
 ...
 * 2. 初始化CAN VCI_InitCAN == 配置CAN通道  VCI_INIT_CONFIG
 * 3. 启动CAN VCI_StartCAN
 * 4. 发送数据 VCI_Transmit
 * 5. 接收数据 VCI_Receive
