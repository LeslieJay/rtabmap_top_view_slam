 ...
 * @LastEditTime: 2025-07-24 15:26:04
...
KeyboardController::KeyboardController() : Node("keyboard_controller")
{
  ...
  // 创建发布者
  speed_pub_ = this->create_publisher<fork_interfaces::msg::DifferentialWheel>("ShaoGuan/AG05D/N38/DifferentialWheel_vel", 10);
}
...
{ 
	...
	// tf2_ros 840但是由于地面不平 当从高处往低处时会发生一点滑行 导致丢吗 如 84-->91,所以把值改小一点
  {
    ...
    auto msg = std::make_shared<fork_interfaces::msg::DifferentialWheel>();
